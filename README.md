...
无需创建 Tag 也可以获取 Windows 快照安装包：推送到 `main` 后，打开仓库的 **Actions → Windows EXE Snapshot**，选择对应运行记录，在页面底部的 **Artifacts** 下载 `coding-tools-mcp-windows-*`，解压后即可得到 `.exe`。快照保留 14 天；正式版本仍从 Releases 下载。
...
需要让 ChatGPT 长期使用同一个 Cloudflare 地址时，参见[固定域名 MCP 部署、诊断与回滚](docs/fixed-domain-mcp.md)。
